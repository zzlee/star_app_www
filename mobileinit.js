...
FmMobile.submitDooh = function(){
    ...
    $.post(url, query, function(res){
           ...
           navigator.notification.alert('≥Ëª‰Â§ßËû¢ÂπïÂ§±);
...
//            FM_LOG("[pushNotification] " + JSON.stringify(result));
              /* result:
                {"notifications":[{"messageFrom":"Miix.tv","applicationStateActive":"0","applicationLaunchNotification":"0","timestamp":1376554348.356165,"aps":{"alert":"®Ê‰∏ÄãÊñ∞ÂΩ±ÁÔº,"sound":"ping.aiff","badge":1}}]
               }
    ...
    onFBConnected: function(){
        ...
        $.post(url, data, function(response){
            ...
            if(response.data){
               ...
               //localStorage.verified='true';//Ê≠§Ë∫‰Ê∏¨Ë©¶ªË©±Ë™çËÔº
...
postFbMessage:function(){
    ...
    name:"ôÊòØ"+localStorage.fb_name+"ÑË©¶°Á∑®+FmMobile.finishNumber+"‰ΩúÂÔºåÊÊ©üÊ®Âè∞óÂ§©ÂπïLED≠Âá∫Ç‰Â§ßËû¢ÂπïAPP¨‰,
        //description:"‰∏äÂ§ß¢ÂAPP ¨‰
...
postFbMessage_live:function(){
   ...
   var post_minute= post_live_time.getMinutes();
    ...
    var timeString=post_year+"Âπ+post_month+"+post_date+"+post_pmAm+post_format_hour+":"+post_minute;
    var timeString_short=post_year+"Âπ+post_month+"+post_date+";
    //----- end of handle time
    ...
    var params_album = {
    access_token: localStorage.fb_accessToken,
    name:"ÂØ¶ÊË®òÈÔº+timeString_short+"ª‰∞ÂÂ§©ÂLED",
    message:FmMobile.userContent.text
    };
    ...
    $.post(url_album,params_album, function(response){
           ...
           message:localStorage.fb_name+"+timeString+"ÔºåÁôª‰∏äÂè∞óÂ§©ÂπïLEDÔºåÊË¨ù‰Á≤æÈÑ‰ÅÔ\n‰∏äÂ§ß¢ÂAPPÁ≤âÁµ≤òÔhttps://www.facebook.com/OnDaScreen"
           
           };
           var params_photo_2={
           access_token: localStorage.fb_accessToken,
           url:FmMobile.longPhoto,
            message:localStorage.fb_name+"+timeString+"ÔºåÁôª‰∏äÂè∞óÂ§©ÂπïLEDÔºåÈÄôÊòØüÂäÁôªÁ¥†ÊÔºåÂ§©ÂπïÂ∞∫ÂØ∏Ô100¨Â∞∫x16¨Â∞∫Ç\n‰∏äÂ§ß¢ÂAPPÁ≤âÁµ≤òÔhttps://www.facebook.com/OnDaScreen"
...
postFbVideoMessage_live:function(){
    ...
    name:localStorage.fb_name+"+timeString+"ÔºåÁôª‰∏äÂè∞óÂ§©ÂπïLEDÔºåÊË¨ù‰Á≤æÈÑ‰ÅÔ",
         description:"‰∏äÂ§ß¢ÂAPPÁ≤âÁµ≤òÔhttps://www.facebook.com/OnDaScreen"
        //picture:FmMobile.srcForMyUgcViewer,
    //privacy:{'value':'SELF'}
...
postCheckinMessage:function(){
    var url = 'https://graph.facebook.com/me/feed';
    var params = {
    name:"Ë∑ØÁË≤¥ÂØ∂∞Ô"+localStorage.fb_name+"Ë∑ØÈ∞ÂÂ∞èÂ∑®ãÔ‰ªñÁìÂç°âÊÉÂú®∞ÂÂ§©ÂLED≠Âá∫Ôºå‰Â§ßËû¢ÂπïAPP ¨‰,
...
FmMobile.showNotification = function(fun){
    ...
    switch(fun){
        case "uploadUgc":
            navigator.notification.confirm("‰∏äÂÇ≥ÂÆåÊÔº, FmMobile.Confirm(), appName, "Á¢∫Â");
            break;
        case "copyUrl":
            navigator.notification.confirm("Â∑≤ËË£ΩÈÄÔº, FmMobile.Confirm(), appName, "Á¢∫Â");
            break;
        case "error":
            navigator.notification.confirm("ØË™§ºÁÔºåËöÁü•ëÂÄëÔ", FmMobile.Confirm(), appName, "Á¢∫Â");
            break;
        case "moreWords":
            navigator.notification.confirm("Ë∂ÖÈÊØèËêÂà∂Â≠óÊï∏!", FmMobile.Confirm(), appName, "Á¢∫Â");
            break;
        case "moreLines":
            navigator.notification.confirm("Ë∂ÖÈ3Ë°åÂÔº, FmMobile.Confirm(), appName, "Á¢∫Â");
            break;
        case "nullText":
            navigator.notification.confirm("Ë´ãËº∏•ÊÂ≠óÔ", FmMobile.Confirm(), appName, "Á¢∫Â");
            break;
        case "newUgc":
            navigator.notification.confirm("®Ê‰∏ÄãÊñ∞ÂΩ±ÁÔº, FmMobile.Confirm(), appName, "Á¢∫Â");
            break;
        case "informLiveTime":
            navigator.notification.confirm("®Áï‰ª∂≥Â‰∏äÊÔº, FmMobile.Confirm(), appName, "Á¢∫Â");
            break;
        case "settingQaSend":
            navigator.notification.confirm("èÈÂ∑≤ÈÄÅÂá∫Ôº, FmMobile.Confirm(), appName, "Á¢∫Â");
            break;
        case "gpsDeny":
            navigator.notification.confirm("ÂÆåÂÖ®Ê≤íÊÂÆö‰, ≥ÈÄöÊË´ãËá≥\nË®≠Â->±Á->ÂÆö‰çÂ\n‰∫§Âá∫‰Ω†ÁêÊÔº, FmMobile.Confirm(), appName, "Á¢∫Â");
            break;
        case "gpsDenyAndroid":
            navigator.notification.confirm("I can't see you...\n È∫ªÁÖ©ãÂ‰∏Ä‰∏ãGPS,ñÂà∞Á©∫Ê∞Ê∂ºÂ§öËÔº, FmMobile.Confirm(), appName, "Á¢∫Â");
            break;
        case "wrongPlace":
            navigator.notification.confirm("∂ÂØ¶‰Ω†‰®ÂÂ∑®ËÂ∞çÂêß...\n∫ÂÂ∏ÇÊùæÂ±±Âó‰∫¨±Ë∑Ø4ÊÆü\nÊ≠°Ë‰æÜÊ°Ô\n(Ë´ãÁßªïÂà∞ºÁãÂ∞Â§©ÂπïÁ∞Êñπ)", FmMobile.Confirm(), appName, "Á¢∫Â");
            break;
        case "inAreaTextOver":
            navigator.notification.confirm("Ë∂ÖÈ‰∏âËâÔÁ¨¨ÂË°åÂú®Â§©Â‰∏äÁ‰∏çÂà∞¶Ô", FmMobile.Confirm(), appName, "Á¢∫Â");
            break;
        case "enableNetwork":
            navigator.notification.confirm("¨ÊôÂàÈÄé•Á∂≤Ë∑ØÔºåËÊ™¢Êü•®ÁÁ∂≤Ë∑Ø∞Â, FmMobile.Confirm(), appName, "Á¢∫Â");
            break;
        case "uploadFailed":
            navigator.notification.confirm("‰∏äÂÇ≥Â§±ÊÔºåËçÊñ∞çË©¶, FmMobile.Confirm(), appName, "Á¢∫Â");
            break;
        case "share":
            navigator.notification.confirm("Ü‰∫´ÂÆåÊÔº, FmMobile.Confirm(), appName, "Á¢∫Â");
            break;
        case "checkIn":
            navigator.notification.confirm("Â∑≤ÊüÂú®Â∞èÂ∑®ãÊ°Ô", FmMobile.Confirm(), appName, "Á¢∫Â");
            break;
        case "sendCode":
            navigator.notification.confirm("Ë™çËÁ¢ºÂ∑≤ÅÂá∫, FmMobile.Confirm(), appName, "Á¢∫Â");
            break;
        case "reSendCode":
            navigator.notification.confirm("Ë´ãÊñº‰∏âÂòÂçÁôºÅ‰Ê¨°ËË≠âÁ¢ºË´ãÊ,Ë¨ùËÔº, FmMobile.Confirm(), appName, "Á¢∫Â");
            break;
        case "wrongPhoneNumber":
            navigator.notification.confirm("ãÊüÁ¢ºÂ•ΩÂ‰∏çÊòØ∑ÈÄôÊ®£..Ë´ãÈ∞Ëº∏, FmMobile.Confirm(), appName, "Á¢∫Â");
            break;
        case "inputWrongCode":
            navigator.notification.confirm("ãÊË™çËÁ¢º‰Â§™Â..Ë´ãÈ∞Ëº∏, FmMobile.Confirm(), appName, "Á¢∫Â");
            break;
        case "waitForCode":
            navigator.notification.confirm("ãÊüÁ¢ºàÊ†ºÔºÅÂç≥Â∞áÁôºÅËË≠âÁ¢º4Á¢º\nË´ãÁôËË¨ùÔ", FmMobile.Confirm(), appName, "Á¢∫Â");
            break;
        case "codeVerifySuccess":
            navigator.notification.confirm("≠ÂªË©±Ë™çËêÂÔºÅ\nÊ∫ñÂ‰∏äÂ§ß¢ÂßÔÔº, FmMobile.Confirm(), appName, "Á¢∫Â");
            break;
        default:
//            console.log("ShowNotification is not worked");
            navigator.notification.confirm(fun, FmMobile.Confirm(), appName, "Á¢∫Â");
    }
